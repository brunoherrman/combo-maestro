...
- Changed (team audit): novo `combo-maestro audit` em `src/audit.js` — resumo unificado do TEAM: atividade/uso por agente (reusa scans do quota: Claude+Codex), delegacoes registradas por status no periodo, e memory do core (via `memory stats`, com `shellCommandLine` p/ evitar DEP0190). AionUI (SQLite) e Modo A in-session ficam de fora (documentado). Teste smoke + assert de sem-DEP
- Changed (cross-agent harvest): `memory harvest` virou multi-agente (`--agent claude|codex|all`, default all). Novo parser de sessao do Codex (`{timestamp,ordinal,type,payload}`, user turn = payload.role=user + content input_text), filtrado pelo `cwd` da sessao = projeto (sem vazar outro projeto); filtra AGENTS.md-injetado. Candidatas levam proveniencia `[claude]`/`[codex]` no `memory record`. Smoke real: extraiu turno do Codex; teste unit de codexUserTurns + filtro de cwd
